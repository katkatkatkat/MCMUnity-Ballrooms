...
public class GameController360fly : MonoBehaviour {

	public static GameController360fly control;
	...
	void Start () {
		// load level 01
		StartCoroutine(LoadLevel("Video01"));
	}
	...
	private IEnumerator UnloadLevels(string exception){
		...
		for (int i = 0; i < SceneManager.sceneCount; i++){
			...
			if (SceneManager.GetSceneAt(i).name != exception && SceneManager.GetSceneAt(i).name != "360fly-video"){
